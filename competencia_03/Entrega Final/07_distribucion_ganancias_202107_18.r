...
#---------------------------------------------------------------------------------------------------------#
#----------------------------------DISTRIBUCION GANANCIAS SEMILLAS---------------------------------------------#


# Calculo la ganancia para cada semilla con envíos fijo segun el calculo anterior

envios<-12000 #setear segun el calculo de ganancias
...
for (i in 1:30) {
  ...
  df_ordenado$prediccion[1:envios] <- 1
...
# Genero 50 ensembles de 20 semillas, tomo muestras con reposición del conjunto de 30 semillas original
...
for (i in 1:num_ensembles) {
  ...
  sampled_seeds <- sample(1:semillas, size = sample_size, replace = TRUE)
